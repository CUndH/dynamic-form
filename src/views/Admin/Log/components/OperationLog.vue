...
const { methods, register, tableObject } = useTable({
  ...
  getListCallback: getDeviceListMapCallback
})

function getDeviceListMapCallback(tableObject, res: IResponse) {
  if (res.code !== 0) {
  ...
  <div :class="`${prefixCls}-list-header`">
    <div :class="`${prefixCls}-list-header-left`">
      <el-input
        ...
        class="mr-10px w-120"
      />
      <el-select v-model="searchKey.module" placeholder="所属模块" class="mr-10px w-120">
        <el-option
      ...
      <el-select v-model="searchKey.type" placeholder="操作类型" class="mr-10px w-120">
        <el-option
      ...
      />
      <el-input class="mr-10px w-120" v-model:value="searchKey.person" placeholder="操作人员" />
